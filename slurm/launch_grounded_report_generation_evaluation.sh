#!/bin/bash
#
# SLURM submission script for running VLM Grounded Report Generation evaluation.
#
# ------------------------------------------------------------------------------
# Usage:
...
#   CONDA_ENV=maira sbatch ./slurm/launch_grounded_report_generation_evaluation.sh maira-2 padchest-gr
#
# Example Usage for a fine-tuned MedGemma:
#   CONDA_ENV=vlm sbatch ./slurm/launch_grounded_report_generation_evaluation.sh medgemma padchest-gr \
#     --medgemma_adapter_path /path/to/your/checkpoint-1000
#
# Note: Currently, 'padchest-gr' is the only supported dataset for this task.
# ------------------------------------------------------------------------------

# --- SLURM Configuration ---
#SBATCH --job-name=vlm-rg-eval
#SBATCH --output=./logs/vlm-rg-eval-%j.out # Feel free to change this to a more specific path.
#SBATCH --nodes=1
...
if [ "$#" -lt 2 ]; then
    ...
    echo "Note: 'dataset' should be 'padchest-gr' for this script."
    exit 1
fi
...
JOB_NAME="eval-rg-${MODEL_NAME}-on-${DATASET_NAME}${JOB_TAG:-"-base"}" # Fallback to "-base"
scontrol update job "$SLURM_JOB_ID" JobName="$JOB_NAME"

# --- Dynamic Log File Redirection ---
# Set a generic log directory
...
echo "Starting Python grounded report generation evaluation script..."
python ./scripts/eval_grounded_report_generation.py \
