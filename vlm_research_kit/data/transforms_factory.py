...
from vlm_research_kit.utils.bbox_utils import BBOX_FORMAT_TO_ALBUMENTATIONS_FORMAT
...
class ConfigurableAlbumentations:
    ...
    def __init__(self,
                 ...
                 ssr_p: float = 0.5,
                 hflip_p: float = 0.0, # No horizontal flipping by default
...
def get_pil_image_only_transforms(
    image_size: Union[int, Tuple[int, int]],
    train: bool = False,
) -> Callable:
    """
    Returns a simple PIL image only transform pipeline: Load image -> Resize

    Args:
        image_size (Union[int, Tuple[int, int]]): Target output image size.
        train (bool): If True, applies training transforms (currently none).
    ...
    # Define transforms
    if train:
        raise NotImplementedError("Training transforms not implemented for pil_image_only yet.")
    else:
        transforms_list = [
            LoadImageFromPath(loader_library='pil', mode='RGB'),
            # Resize using PIL
        ...
        composed_transform = v2.Compose(transforms_list)
        def final_transform(image_path: str, **kwargs) -> Dict[str, Any]:
            pil_img = composed_transform(image_path)
            return {'pixel_values': pil_img, **kwargs}
        return final_transform


def get_pil_with_augmentations_transforms(
    image_size: Union[int, Tuple[int, int]],
    is_train: bool = False,
    ...
    """
    Returns a transform pipeline that performs augmentations but outputs a PIL image.
    This version is BBOX-AWARE and is suitable for object detection/grounding tasks.

    Pipeline: Load Image (as NumPy) -> Albumentations (on image + bboxes) -> Convert back to PIL.

    Args:
        image_size: Target output image size.
        is_train: If True, applies training augmentations.
        augmenter_override_params: Dictionary to override default augmentation params.
        bbox_format: The format of the bounding boxes (e.g., 'cxcywh', 'xyxy').

    Returns:
        A callable that accepts image_path, bboxes, bbox_labels and returns a dict
        containing the processed PIL image and augmented bbox data.
    """
    logger.info(
        "Building PIL-output pipeline with Albumentations "
        f"(is_train={is_train}, bbox_format={bbox_format})"
    )

    # 1. Image Loader (use cv2 as it's the native format for Albumentations)
    image_loader = LoadImageFromPath(loader_library="cv2", mode="RGB")
    
    # 2. Convert user-friendly bbox_format to Albumentations format
    alb_bbox_format = None
    if bbox_format:
        alb_bbox_format = BBOX_FORMAT_TO_ALBUMENTATIONS_FORMAT.get(bbox_format)
        if alb_bbox_format is None:
            raise ValueError(f"Unsupported bbox_format string: '{bbox_format}'")

    # 3. Albumentations Augmenter
    override_params = (
        augmenter_override_params if augmenter_override_params is not None else {}
    )
    ...
    try:
        ...
        logger.error("Error initializing ConfigurableAlbumentations: %s", e)
        raise ValueError(f"Invalid parameter for ConfigurableAlbumentations: {e}") from e

    # Build the augmentation pipeline, passing the correct bbox format
    tf_augs = augmenter.build_pipeline(
        is_train=is_train, bbox_format=alb_bbox_format
    )
    tf_augs_no_bbox = augmenter.build_pipeline(
        is_train=is_train, bbox_format=None
    )
    # 4. Define the final wrapper function that handles kwargs
    def final_transform(
        image_path: str,
        ...
        **kwargs
        ...
        try:
            # Load image -> NumPy HWC RGB
            image_np = image_loader(image_path)

            # Prepare input for Albumentations
            alb_input = {"image": image_np}
            actual_tf_augs = tf_augs_no_bbox
            if alb_bbox_format and bboxes is not None and bbox_labels is not None:
                alb_input["bboxes"] = bboxes
                alb_input["bbox_labels"] = bbox_labels
                actual_tf_augs = tf_augs
            ...
            pil_img = Image.fromarray(augmented_image_np)

            # Prepare final output dictionary
            output_dict = {"pixel_values": pil_img}
            
            # Pass through the (potentially augmented) bounding boxes
            if "bboxes" in augmented_data:
                output_dict["bboxes"] = augmented_data["bboxes"]
                output_dict["bbox_labels"] = augmented_data["bbox_labels"]
            # If no augmentation was done on bboxes, pass them through unchanged
            elif bboxes is not None:
                output_dict["bboxes"] = bboxes
                output_dict["bbox_labels"] = bbox_labels

            # Include any other keyword arguments that were passed in
            ...
            logger.error(
                "Error during pil_with_augmentations transform for %s: %s",
...
def create_image_transforms(
    ...
    if use_model_specific_transforms:
        ...
        elif model_name == "pil_image_only":
            if image_size is None:
                raise ValueError("image_size must be provided for pil_image_only transforms.")
            return get_pil_image_only_transforms(image_size=image_size, train=is_train)
        elif model_name == "pil_with_augmentations":
            if image_size is None:
                raise ValueError(
                    "image_size must be provided for pil_with_augmentations transforms."
                )
            return get_pil_with_augmentations_transforms(
                image_size=image_size,
                is_train=is_train,
                augmenter_override_params=augmenter_override_params,
        ...
        if bbox_format is not None:
            bbox_format = BBOX_FORMAT_TO_ALBUMENTATIONS_FORMAT[bbox_format] # Convert to Albumentations format
            ...
            **kwargs
            ...
            try:
                ...
                if bbox_format is not None:
                    ...
                    if bboxes is not None: # If bboxes were provided, add them to output
                        output_dict['bboxes'] = bboxes
                    if bbox_labels is not None: # If bbox_labels were provided, add them to output
                        output_dict['bbox_labels'] = bbox_labels
